...
import roshanmanjaly.cs301.cs.wm.edu.amazebyroshanmanjaly.ui.generation.Factory;
...
public class MazeController implements Order {
    ...
    private RangeSet rset=new RangeSet();
    ...
    private double robotBattery=0;
    private int robotDistance=0;
    ...
    public boolean hasWall(int x, int y, CardinalDirection cd) {
        return mazeConfig.hasWall(x,y,cd);
    }
    /**
    ...
    private void operateGameInPlayingState(UserInput key) {
        ...
        switch (key) {
            ...
            case Up: // move forward
                walk(1);
                if (isOutside(px,py) || driver.getRobot().hasStopped()) {
    ...
    public void setRobotDistance(int dist) {
        robotDistance=dist;
    }

    public void setRobotBattery(float batt) {
        robotBattery=batt;
    }
    public int getRobotDistance() {
        return robotDistance;
    }
    public double getRobotBattery() {
        return robotBattery;
    }
